  ...
  if (form && card) {
    ...
    form.addEventListener('submit', function (e) {
      ...
      fetch(FORM_ENDPOINT, {
        ...
        /* Слишком часто с одного адреса — единственный случай, когда
           посетителю честнее показать сообщение, а не слать письмо. */
        if (r.status === 429) {
          return fail('Вы уже отправили несколько заявок. Позвоните нам: ' + FORM_PHONE + '.');
        }

        /* Всё остальное — обработчик недоступен, отвалился или ответил
           ошибкой. Заявку не теряем: уводим письмом, посетителю ошибку
           не показываем. */
        byMail();
      }).catch(function () {
        byMail();
